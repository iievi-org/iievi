...
7. **Circuit breakers wrap every external API call** (Gemini, Meta, WhatsApp, TikTok, LinkedIn, Razorpay, Stripe) via `app/core/circuit.py` — a Redis-backed implementation (threshold 5, recovery 60s) so ALL worker processes share one circuit state; the in-process `circuitbreaker` library was deliberately not used.
