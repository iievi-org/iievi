...
export const Route = createFileRoute("/_marketing/register")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Create your IIEVI account in 30 seconds. No credit card, 14-day free trial of all features." },
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 min-h-[calc(100vh-5rem)]">
          ...
          <div className="mt-10">
            <ChatMockup
              ...
              messages={[
                ...
                { from: "ai", text: "Yes! 3 PM, 5 PM, or 6:30 PM are open. Which works best?", time: "Now" },
                { from: "user", text: "5 PM please", time: "Now" },
                { from: "ai", text: "Booked ✓ See you at 5 PM today. Reminder 30 min before.", time: "Now" },
              ]}
            />
          </div>
        </div>
      ...
      <div className="flex items-center justify-center p-8 md:p-16">
        <div className="w-full max-w-md">
          <Link to="/" className="font-display font-bold text-[22px] tracking-[-0.01em] text-ink">IIEVI</Link>
          {done ? (
            <div className="mt-12">
              ...
              <p className="mt-4 text-body-md text-graphite">
                Check your inbox at <span className="text-ink">{form.email}</span> for your dashboard link.
                Our team will WhatsApp you in 5 minutes to set up your number.
              </p>
              <div className="mt-8">
                <Link to="/" className="font-body text-label-sm uppercase tracking-[0.14em] text-ink border-b border-hairline pb-1">
                  Back to home
                </Link>
              </div>
            </div>
            ...
            <>
              ...
              <form onSubmit={submit} className="flex flex-col gap-6">
                ...
                <Input label="Business name" name="business" value={form.business} onChange={(e) => setForm({ ...form, business: e.target.value })} error={errors.business} />
                <Input label="Email" type="email" name="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} error={errors.email} />
                <Input label="WhatsApp number" name="phone" placeholder="+91 98765 43210" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} error={errors.phone} />
                <Button type="submit" variant="primary" className="mt-4 w-full">Create Account</Button>
                <p className="font-mono text-mono-sm text-stone">
                  By signing up you agree to our <Link to="/terms" className="text-ink border-b border-hairline">Terms</Link> and <Link to="/privacy" className="text-ink border-b border-hairline">Privacy Policy</Link>.
                </p>
              </form>
            </>
          )}
